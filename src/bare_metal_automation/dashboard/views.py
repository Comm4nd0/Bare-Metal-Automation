"""Views for Bare Metal Automation dashboard."""
...
def dashboard(request):
    ...
    netbox_configured = bool(
        getattr(django_settings, "BMA_NETBOX_URL", ""),
    )
    ...
    return render(request, "dashboard/index.html", {
        ...
        "deployment_control": deployment_status(),
        "rollback_control": rollback_status(),
    })
...
def api_services(request):
    """GET /api/services/ — check laptop service statuses."""
    from bare_metal_automation.common.services import get_laptop_services

    services = get_laptop_services()
    return JsonResponse({
        "services": [
            {
                "name": s.name,
...
@require_GET
def api_deployment_control_status(request):
    """GET /api/deployment/status/ — check deployment runner state."""
...
@require_GET
def api_rollback_control_status(request):
    """GET /api/rollback/status/ — check rollback runner state."""
    return JsonResponse(rollback_status())
...
def api_prepare_nodes(request):
    ...
    try:
        ...
        tag_pattern = getattr(
            django_settings, "BMA_NETBOX_TAG_PATTERN", r"^D\d+$",
        )
        client = NetBoxClient(
            url=netbox_url,
            token=getattr(django_settings, "BMA_NETBOX_TOKEN", ""),
        )
        nodes = client.list_node_tags(tag_pattern)
        return JsonResponse({
            "nodes": nodes,
...
def api_start_prepare(request):
    ...
    if not node_tag:
        return JsonResponse(
            {"status": "error", "error": "node_tag is required"},
            status=400,
        )
    result = start_prepare(node_tag)
    status_code = 200 if result["status"] == "started" else 409
    return JsonResponse(result, status=status_code)
...
@require_GET
def api_prepare_status(request):
    """GET /api/prepare/status/ — check preparation progress."""
    return JsonResponse(prepare_status())
