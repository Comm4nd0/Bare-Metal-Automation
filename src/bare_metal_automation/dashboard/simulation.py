...
if TYPE_CHECKING:
    from bare_metal_automation.dashboard.models import Deployment
...
class SimulationEngine:
    ...
    def _log(self, level: str, phase: str, message: str) -> None:
        from bare_metal_automation.dashboard.models import DeploymentLog
    ...
    def _update_device(self, hostname: str, **fields: object) -> None:
        from bare_metal_automation.dashboard.models import Device
    ...
    def _run_phases(self) -> None:
        ...
        # Pause briefly, then run rollback simulation
        if not self._sleep(3):
            return

        rollback_phases = [
            self._phase_rollback_pre_flight,
            self._phase_rollback_ntp_reset,
            self._phase_rollback_server_reset,
            self._phase_rollback_laptop_pivot,
            self._phase_rollback_network_reset,
            self._phase_rollback_final_check,
        ]

        for phase_fn in rollback_phases:
            if _simulation_stop.is_set():
                self._log("WARNING", "simulation", "Stopped by user")
                dep = self._dep
                dep.phase = "stopped"
        ...
        self._set_phase("rollback_complete")
        self._log(
            "INFO", "rollback_complete",
            "All devices reset to factory state — kit ready for next cycle",
        )
    ...
    def _phase_cabling_validation(self) -> bool:
        from bare_metal_automation.dashboard.models import CablingResult, Device
    ...
    def _phase_rollback_pre_flight(self) -> bool:
        self._set_phase("rollback_pre_flight")
        self._log(
            "WARNING", "rollback_pre_flight",
            "ROLLBACK TO FACTORY initiated — all devices will be reset",
        )
        if not self._sleep(1):
            return False
        self._log("INFO", "rollback_pre_flight", "Verifying connectivity to all devices...")
        if not self._sleep(1):
            return False
        self._log("INFO", "rollback_pre_flight", f"All {len(SIMULATED_DEVICES)} devices reachable")
        return True

    def _phase_rollback_ntp_reset(self) -> bool:
        self._set_phase("rollback_ntp_reset")
        ntp_devices = self._get_ntp_devices()
        self._log(
            "INFO", "rollback_ntp_reset",
            f"Factory-resetting {len(ntp_devices)} NTP device(s)...",
        )
        ...
        for device_data in ntp_devices:
            if _simulation_stop.is_set():
                return False
            hostname = str(device_data["intended_hostname"])
            self._update_device(hostname, state="resetting")
            self._log("INFO", "rollback_ntp_reset", f"{hostname}: reverting configuration...")
            if not self._sleep(1):
                return False
            self._log("INFO", "rollback_ntp_reset", f"{hostname}: rebooting to factory defaults...")
            if not self._sleep(1):
                return False
            self._update_device(hostname, state="factory_reset")
            self._log("INFO", "rollback_ntp_reset", f"{hostname}: factory reset complete")

        return True

    def _phase_rollback_server_reset(self) -> bool:
        self._set_phase("rollback_server_reset")
        hpe_devices = self._get_hpe_devices()
        self._log(
            "INFO", "rollback_server_reset",
            f"Factory-resetting {len(hpe_devices)} HPE server(s) via Redfish...",
        )
        ...
        for device_data in hpe_devices:
            ...
            hostname = str(device_data["intended_hostname"])
            self._update_device(hostname, state="resetting")
            self._log("INFO", "rollback_server_reset", f"{hostname}: resetting BIOS to defaults...")
            if not self._sleep(0.5):
                return False
            self._log(
                "INFO", "rollback_server_reset",
                f"{hostname}: deleting RAID logical drives...",
            )
            if not self._sleep(0.5):
                return False
            self._log("INFO", "rollback_server_reset", f"{hostname}: ejecting virtual media...")
            if not self._sleep(0.3):
                return False
            self._log("INFO", "rollback_server_reset", f"{hostname}: resetting iLO to factory...")
            if not self._sleep(0.5):
                return False
            self._update_device(hostname, state="factory_reset")
            self._log("INFO", "rollback_server_reset", f"{hostname}: powering off server...")
            if not self._sleep(0.5):
                return False
            self._update_device(hostname, state="powered_off")
            self._log("INFO", "rollback_server_reset", f"{hostname}: powered off")

        return True

    def _phase_rollback_laptop_pivot(self) -> bool:
        self._set_phase("rollback_laptop_pivot")
        self._log(
            "INFO", "rollback_laptop_pivot",
            "Reconfiguring laptop NIC back to bootstrap VLAN (10.255.0.0/16)...",
        )
        if not self._sleep(1.5):
            return False
        self._log(
            "INFO", "rollback_laptop_pivot",
            "Laptop NIC assigned 10.255.255.1 on bootstrap network",
        )
        if not self._sleep(0.5):
            return False
        return True

    def _phase_rollback_network_reset(self) -> bool:
        self._set_phase("rollback_network_reset")
        cisco_devices = self._get_cisco_devices()
        self._log(
            "INFO", "rollback_network_reset",
            f"Factory-resetting {len(cisco_devices)} network device(s) (outside-in)...",
        )

        # Group by depth, process outside-in (same as deployment)
        depths = sorted(
            {int(d["bfs_depth"]) for d in cisco_devices}, reverse=True,
        )
        ...
        for depth in depths:
            ...
            self._log(
                "INFO", "rollback_network_reset",
                f"Resetting depth {depth} devices ({len(depth_devices)} device(s))...",
            )
            ...
            for device_data in depth_devices:
                ...
                self._log(
                    "INFO", "rollback_network_reset",
                    f"{hostname}: factory reset — device reloading",
                )
    ...
    def _phase_rollback_final_check(self) -> bool:
        self._set_phase("rollback_final_check")
        self._log("INFO", "rollback_final_check", "Verifying factory reset status...")
        if not self._sleep(1):
            return False

        cisco_count = len(self._get_cisco_devices())
        hpe_count = len(self._get_hpe_devices())
        ntp_count = len(self._get_ntp_devices())

        self._log(
            "INFO", "rollback_final_check",
            f"{cisco_count} network devices: factory reset verified",
        )
        if not self._sleep(0.5):
            return False
        self._log(
            "INFO", "rollback_final_check",
            f"{hpe_count} HPE servers: powered off",
        )
        if not self._sleep(0.5):
            return False
        self._log(
            "INFO", "rollback_final_check",
            f"{ntp_count} NTP devices: factory reset verified",
        )
        if not self._sleep(0.5):
            return False
        self._log(
            "INFO", "rollback_final_check",
            f"All {len(SIMULATED_DEVICES)} devices at factory state — "
            "kit ready for next deployment cycle",
        )
        return True
...
def start_simulation(name: str = "SIM-Rack-Demo") -> dict[str, str]:
    ...
    # Mutual exclusion with deployment and rollback
